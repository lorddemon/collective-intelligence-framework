...
my $url = 'https://spyeyetracker.abuse.ch/monitor.php?rssfeed=configurls';
...
foreach my $item (@{$rss->{items}}){
    $_ = $item->{'description'};
    my ($url,$status,$md5) = m/^SpyEye ConfigURL: (\S+), Status: (\S+), MD5 hash: (\S+)/;
    ...
    my $impact = 'malware spyeye config';
    ...
    $impact = 'malware url spyeye config';
